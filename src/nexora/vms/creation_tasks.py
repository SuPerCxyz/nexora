"""Persistent VM creation task contract and handler."""
...
class VmCreationTaskInput:
    ...
    @classmethod
    def decode(cls, value: str | None) -> "VmCreationTaskInput":
        if value is None or len(value) > 1_024:
            raise ValueError("VM creation task input is unavailable")
        try:
            ...
            raise ValueError("VM creation task input is invalid") from exc
        if not result.plan_id or not result.host_id:
            raise ValueError("VM creation task input is invalid")
        return result


@dataclass
class VmCreationHandler:
    service: VmCreationService

    def __call__(self, context: TaskContext, task: Task) -> str:
        task_input = VmCreationTaskInput.decode(task.input_summary)
        if (
            task.task_type != "vm.create"
            or task.host_id != task_input.host_id
            or task.vm_uuid != task_input.vm_uuid
        ):
            raise ValueError("VM creation task scope does not match input")
