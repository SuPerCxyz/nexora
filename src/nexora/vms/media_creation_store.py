"""Transaction-scoped platform-image VM plan state changes."""

from datetime import UTC, datetime

from nexora.db import Database
from nexora.vms.change_models import VmChangePlanStatus
from nexora.vms.media_creation_models import VmMediaCreationPlan


class VmMediaCreationPlanStore:
    def __init__(self, database: Database) -> None:
        self.database = database

    def load_for_execution(self, plan_id: str) -> VmMediaCreationPlan:
        with self.database.session() as session:
            plan = session.get(VmMediaCreationPlan, plan_id)
            if plan is None or plan.status not in {
                VmChangePlanStatus.CONFIRMED,
                VmChangePlanStatus.RUNNING,
            }:
                raise ValueError("platform-image VM plan cannot be executed")
            return plan

    def mark_running(self, plan_id: str) -> None:
        with self.database.session() as session:
            plan = session.get(VmMediaCreationPlan, plan_id)
            if plan is None or plan.status not in {
                VmChangePlanStatus.CONFIRMED,
                VmChangePlanStatus.RUNNING,
            }:
                raise ValueError("platform-image VM plan state changed")
            plan.status = VmChangePlanStatus.RUNNING

    def mark_succeeded(self, plan_id: str, resource_id: str) -> None:
        with self.database.session() as session:
            plan = session.get(VmMediaCreationPlan, plan_id)
            if plan is not None:
                plan.status = VmChangePlanStatus.SUCCEEDED
                plan.result_resource_id = resource_id
                plan.finished_at = datetime.now(UTC)

    def record_resized_hash(self, plan_id: str, digest: str) -> None:
        if len(digest) != 64:
            raise ValueError("resized image hash is invalid")
        with self.database.session() as session:
            plan = session.get(VmMediaCreationPlan, plan_id)
            if plan is None or plan.status != VmChangePlanStatus.RUNNING:
                raise ValueError("platform-image VM plan is not running")
            if plan.resized_image_sha256 not in {None, digest}:
                raise ValueError("resized image hash changed")
            plan.resized_image_sha256 = digest

    def mark_failed(self, plan_id: str, error: str) -> None:
        with self.database.session() as session:
            plan = session.get(VmMediaCreationPlan, plan_id)
            if plan is not None:
                plan.status = VmChangePlanStatus.FAILED
                plan.error_message = error[:2_048]
                plan.finished_at = datetime.now(UTC)
