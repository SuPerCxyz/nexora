...
export function MediaPage() {
  ...
  async function scan() {
    setScanning(true);
    try { navigateToTask((await scanMedia()).location); }
...
function MediaActions({ item, issuing, onIssue }: { item: MediaItemSummary; issuing: boolean; onIssue: () => void }) {
  ...
  return <Space><Button size="small" className="nx-btn-info" href={`/media/${item.id}/copy`}>复制到节点</Button>{item.standalone && <Button size="small" type="primary" href="/vms/create/platform-image">创建虚拟机</Button>}</Space>;
}
