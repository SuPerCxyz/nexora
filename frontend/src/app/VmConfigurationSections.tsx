...
export function NetworkConfiguration({ data, preview }: Props) {
  ...
  return <Space orientation="vertical" size={12} className="nx-page-stack">
    <Card title="当前网络接口"><Table className="nx-responsive-table" dataSource={interfaces} pagination={false} tableLayout="fixed" columns={[
    ...
    </div><Button htmlType="submit" type="primary" disabled={!data.networks.length}>保存网卡</Button></Form></Card>
...
export function StorageConfiguration({ data, preview }: Props) {
  ...
  async function createVolume() {
    ...
    setCreating(true);
    try { navigateToTask((await applyStorageVolume(volumePreview)).location); }
    catch (caught) { message.error(caught instanceof Error ? caught.message : "创建卷任务失败"); setCreating(false); }
  }
  return <Space orientation="vertical" size={12} className="nx-page-stack">
    <Card title="当前磁盘与光驱"><Table className="nx-responsive-table" dataSource={disks} pagination={false} tableLayout="fixed" columns={[
    ...
    <Card title="挂载同节点存储卷" extra={<Button size="small" type="primary" onClick={() => { volumeForm.resetFields(); setVolumePreview(null); setCreateOpen(true); }}>新建卷</Button>}>
      {Object.entries(poolGroups).map(([poolName, items]) => (
        <div className="nx-pool-group" key={poolName}>
          ...
          <Table className="nx-responsive-table" rowKey="resource_id" dataSource={items} pagination={false} tableLayout="fixed" columns={[
    ...
    <Modal title="新建存储卷" open={createOpen} onCancel={() => setCreateOpen(false)} footer={null} width={520}>
      {volumePreview ? (
        <Space orientation="vertical" size={12} className="nx-page-stack">
          ...
          <Flex justify="flex-end" gap={8}><Button onClick={() => setVolumePreview(null)}>返回修改</Button><Button type="primary" loading={creating} onClick={createVolume}>确认并创建任务</Button></Flex>
        </Space>
      ) : (
