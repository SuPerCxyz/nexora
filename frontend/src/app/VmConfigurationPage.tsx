...
export function VmConfigurationPage({ hostId, vmId }: { hostId: string; vmId: string }) {
  ...
  const saveChange = async (operation: string, values: Record<string, unknown>) => {
    try { navigateToTask((await saveVmConfiguration(data, operation, values)).location); }
  ...
  async function doRollback(item: VmXmlHistoryItem) {
    setRollbacking(true);
    try { navigateToTask((await rollbackVmConfiguration(hostId, vmId, item.id)).location); }
  ...
  return <Space orientation="vertical" size={12} className="nx-page-stack">
    <Flex className="nx-detail-header" justify="space-between" align="start" gap={16} wrap>
      <div className="nx-page-title"><Flex align="center" gap={12} wrap><Typography.Title level={2}>{data.vm.name} · 配置</Typography.Title><VmStatusTag state={data.vm.state} /></Flex><Typography.Text type="secondary">修改后直接保存进入任务队列；每次保存前自动记录前一版 XML，可随时回滚</Typography.Text></div>
    ...
    {!data.vm.persistent && <Alert type="warning" showIcon title="临时虚拟机不支持持久化配置修改" />}
    {data.vm.active && <Alert type="info" showIcon title="虚拟机正在运行：CPU / 内存 / 高级配置的修改会写入持久化配置，需重启虚拟机后生效（保存后显示待重启标记）；磁盘、网卡与光驱热插拔会在运行中即时生效。" />}
    ...
    <Modal title="配置历史版本" open={historyOpen} onCancel={() => setHistoryOpen(false)} footer={null} width={600}>
      {historyError ? <Alert type="error" showIcon title={historyError} /> : history === null ? <PageLoading /> : history.length === 0 ? <PageEmpty description="尚无配置历史快照；保存配置时会自动记录前一个版本" /> : (
        <Table className="nx-responsive-table" rowKey="id" dataSource={history} pagination={false} tableLayout="fixed" columns={[
...
function ComputeForms({ data, preview }: { data: VmConfiguration; preview: (operation: string, values: Record<string, unknown>) => void }) {
  ...
  return <Space orientation="vertical" size={12} className="nx-page-stack">
    <Card title="CPU 拓扑"><Form form={cpuForm} layout="vertical" initialValues={cpu ?? {}} onFinish={(values) => preview("cpu", values)}><div className="nx-form-grid-4">{[
      ...
      {topoExceeds && <Alert type="warning" showIcon title={`拓扑乘积 ${topoProduct} 超过最大 vCPU ${maxVcpus}，请减少 Sockets/Dies/Cores/Threads 或增大最大 vCPU`} />}
      {!topoExceeds && topoProduct < maxVcpus && <Alert type="info" showIcon title={`当前拓扑乘积为 ${topoProduct}，小于最大 vCPU ${maxVcpus}（预留热插拔余量合法）`} />}
      <Flex justify="flex-end" className="nx-form-actions-end"><Button htmlType="submit" type="primary" disabled={!cpu || !data.vm.persistent || topoExceeds}>保存 CPU</Button></Flex></Form></Card>
    ...
    </div><Flex gap={24} wrap><Toggle name="hugepages" label="HugePages" /><Toggle name="locked" label="Locked memory" /><Toggle name="discard" label="Discard" /></Flex><Flex justify="flex-end" className="nx-form-actions-end"><Button htmlType="submit" type="primary" disabled={!memory || !data.vm.persistent}>保存内存</Button></Flex></Form></Card>
  </Space>;
}
