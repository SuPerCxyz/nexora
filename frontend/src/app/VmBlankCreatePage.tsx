...
import type { VmBlankCreateOptions, VmBlankCreatePreview } from "../api/contracts";
import {
  applyVmBlankCreate,
...
export function VmBlankCreatePage() {
  const [options, setOptions] = useState<VmBlankCreateOptions | null>(null);
  ...
  const [preview, setPreview] = useState<VmBlankCreatePreview | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const [actionError, setActionError] = useState<string | null>(null);

  useEffect(() => {
    loadVmBlankCreateOptions().then(setOptions).catch(setError);
  }, []);

  const hosts = useMemo(() => {
    const values = new Map<string, string>();
    options?.pools.forEach((item) => values.set(item.host_id, item.host_name));
    return Array.from(values, ([value, label]) => ({ value, label }));
  }, [options]);
  const pools = useMemo(
    () => options?.pools.filter((item) => item.host_id === hostId) ?? [],
    [options, hostId],
  );
  const networks = useMemo(
    () => options?.networks.filter((item) => item.host_id === hostId) ?? [],
    [options, hostId],
  );
  const isos = useMemo(
    () => options?.isos.filter((item) => item.host_id === hostId) ?? [],
    [options, hostId],
  );
  ...
  async function submitPreview(event: React.FormEvent<HTMLFormElement>) {
    ...
    setActionError(null);
    try {
      setPreview(await previewVmBlankCreate({
        name: String(values.get("name") ?? ""),
        memory_mib: Number(values.get("memory_mib")),
        vcpus: Number(values.get("vcpus")),
        pool_resource_id: poolId,
        disk_name: String(values.get("disk_name") ?? ""),
        volume_format: volumeFormat,
        capacity_gib: Number(values.get("capacity_gib")),
      ...
      setActionError(caught instanceof Error ? caught.message : "空白磁盘创建计划预览失败");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function confirmPreview() {
    ...
    setActionError(null);
    try {
      const task = await applyVmBlankCreate(preview);
      window.location.assign(task.location);
    } catch (caught) {
      setActionError(caught instanceof Error ? caught.message : "空白磁盘创建任务失败");
      setSubmitting(false);
    }
  }
    ...
    <Space className="nx-page-stack nx-create-page" orientation="vertical" size={24}>
      <div className="nx-detail-header">
        <Space orientation="vertical" size={6}>
          <Button type="link" href="/vms/create" icon={<ArrowLeftOutlined />} className="nx-back-link">
            返回虚拟机
          </Button>
          <Typography.Title level={2}>创建虚拟机（空白磁盘）</Typography.Title>
          <Typography.Text type="secondary">
            先创建指定容量的空白磁盘，再定义虚拟机。提交前会展示完整配置差异。
          </Typography.Text>
        </Space>
      </div>
      ...
      <form onSubmit={submitPreview}>
        <input type="hidden" name="pool_resource_id" value={poolId} />
        ...
        <input type="hidden" name="volume_format" value={volumeFormat} />
        ...
        <Space className="nx-create-sections" orientation="vertical" size={16}>
          ...
          <Card title="空白磁盘" extra={<CloudServerOutlined />}>
            <div className="nx-form-grid nx-form-grid-wide">
              <Field label="目标存储池" hint="空白磁盘将创建在所选存储池中。">
                <Select aria-label="目标存储池" showSearch value={poolId || undefined} onChange={setPoolId} disabled={!selectedHostId(hostId)} placeholder="选择可写存储池" optionFilterProp="label" options={pools.map((item) => ({ value: item.id, label: `${item.pool_name} · ${item.target_path}` }))} />
              </Field>
              <Field label="磁盘名称" hint="字母或数字开头，可包含 . _ -">
                <Input aria-label="磁盘名称" name="disk_name" required maxLength={255} pattern="[A-Za-z0-9][A-Za-z0-9_.-]{0,254}" placeholder="例如 web-disk-01.qcow2" />
              </Field>
              <Field label="格式">
                <Select aria-label="磁盘格式" value={volumeFormat} onChange={setVolumeFormat} options={[{ value: "qcow2", label: "qcow2" }, { value: "raw", label: "raw" }]} />
              </Field>
              <Field label="容量（GiB）">
                <InputNumber aria-label="容量（GiB）" name="capacity_gib" min={1} max={8192} defaultValue={20} required />
              </Field>
            </div>
          </Card>
          ...
          <Card title="网络与安装介质">
            <div className="nx-form-grid nx-form-grid-wide">
              <Field label="网络" hint="仅显示与所选存储池相同节点的可用网络。">
                <Select aria-label="网络" allowClear value={networkId || undefined} onChange={(value) => setNetworkId(value ?? "")} disabled={!selectedHostId(hostId)} placeholder="暂不添加网卡" options={networks.map((item) => ({ value: item.id, label: `${item.kind === "bridge" ? "Bridge" : "libvirt Network"} / ${item.name}` }))} />
              </Field>
              <Field label="安装 ISO">
                <Select aria-label="安装 ISO" allowClear value={isoId || undefined} onChange={(value) => setIsoId(value ?? "")} disabled={!selectedHostId(hostId)} placeholder="不挂载安装 ISO" options={isos.map((item) => ({ value: item.id, label: `${item.pool_name} / ${item.name}` }))} />
              </Field>
              <Field label="VirtIO Driver ISO（Windows）">
                <Select aria-label="VirtIO Driver ISO" allowClear value={driverIsoId || undefined} onChange={(value) => setDriverIsoId(value ?? "")} disabled={!selectedHostId(hostId)} placeholder="不挂载 Driver ISO" options={isos.map((item) => ({ value: item.id, label: `${item.pool_name} / ${item.name}` }))} />
              </Field>
            </div>
          </Card>

          <div className="nx-form-actions">
            <Typography.Text type="secondary">将先创建空白磁盘再定义虚拟机，执行前仍需确认配置差异。</Typography.Text>
            <Button type="primary" htmlType="submit" loading={submitting} disabled={!poolId}>预览创建计划</Button>
          </div>
        </Space>
      </form>
      )}
...
function selectedHostId(value: string): boolean {
...
function VmBlankCreatePreviewPanel({
  preview,
  loading,
  error,
  onBack,
  ...
  return (
    <Space className="nx-create-sections" orientation="vertical" size={16}>
      ...
      <Alert type="warning" showIcon title="确认后将创建持久化任务" description="先创建空白磁盘，再定义虚拟机，不会启动虚拟机或修改现有磁盘内容。" />
      <div className="nx-form-actions">
...
function firmwareLabel(summary: VmBlankCreatePreview["summary"]): string {
