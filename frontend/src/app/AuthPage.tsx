...
export function AuthPage({ mode, error, csrf }: { mode: string; error: string | null; csrf: string }) {
  ...
  return <main className="nx-auth-page"><Card className="nx-auth-card">
    <div className="nx-auth-brand"><NexoraIcon size={56} /><Typography.Title level={1}>Nexora</Typography.Title></div>
    <Typography.Title level={2} className="nx-auth-title">{initialize ? "创建本地管理员" : "管理员登录"}</Typography.Title>
    ...
    <Form layout="vertical" onFinish={submit}>
      ...
      <Form.Item label="密码" name="password" rules={[{ required: true }, ...(initialize ? [{ min: 8, message: "密码至少 8 位" }] : [])]}><Input.Password autoComplete={initialize ? "new-password" : "current-password"} /></Form.Item>
